  ...
  const importRecipe = async () => {
    ...
    try {
      ...
      if (platform.type === 'youtube') {
        ...
        try {
          // Normalize Shorts URL to regular watch URL for fetching
          const fetchUrl = url.includes('/shorts/') 
            ? url.replace('/shorts/', '/watch?v=')
            : url;
          const r = await fetch('/api/fetch-url', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: fetchUrl }) });
          const j = await r.json();
          ...
          if (!pageContent && descM) {
            pageContent = `Title: ${titleM?.[1] ?? ''}\nDescription: ${descM[1].replace(/\\n/g,'\n').slice(0,2000)}`;
          }
          if (!pageContent && titleM) {
            pageContent = `Title: ${titleM[1]}`;
          }
        } catch { /* continue */ }
      }
      ...
      const videoId = url.match(/(?:shorts\/|watch\?v=|youtu\.be\/)([^&\s?]+)/)?.[1] ?? '';
      const sch = schema(url, platform.type);

      let prompt: string;
      if (pageContent.length > 40 && platform.type === 'web') {
        prompt = `Extract the exact recipe from this page.\n\nURL: ${url}\n\nCONTENT:\n${pageContent}\n\nReturn ONLY this JSON (no markdown):\n${sch}\n\nRULES:\n- Every ingredient MUST have an exact measurement.\n- Calculate macros accurately by summing each ingredient. Spices/salt/pepper = 0 calories. Cross-check: calories ≈ (protein×4)+(carbs×4)+(fat×9).`;
      } else {
        prompt = `You are extracting a recipe from this ${platform.name} video: ${url}${creator ? ` by @${creator}` : ''}${videoId ? ` (video ID: ${videoId})` : ''}.
${pageContent ? `\n\nContent found from the video:\n${pageContent}` : ''}

Search for this specific video to identify exactly what dish is being made. Look up the video title, creator, and any associated recipe.

IMPORTANT: Even if you cannot find the exact recipe online, you MUST still return a complete recipe JSON. Use your best judgment based on:
- The video title or caption if available
- The creator's known cooking style
- Any ingredients or techniques visible/mentioned

Never return an error — always generate the most accurate recipe you can.

Return ONLY this JSON (no markdown):\n${sch}\n\nRULES:\n- Every ingredient MUST have an exact measurement. Estimate realistic amounts if not specified.\n- Calculate macros accurately by summing each ingredient. Spices/salt/pepper = 0 calories.\n- Set "desc" to note if this is an estimate: e.g. "Estimated recipe based on video content."`;
      }
      ...
      if (!recipe?.title) throw new Error('Could not extract recipe. Try a different link or paste the recipe URL directly.');
